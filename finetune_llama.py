...
def preprocess_triviaqa(example, tokenizer, max_length=2048):
    instruction = f"### Instruction:\n{example['question']}\n\n### Response:\n"
...
def main(args):
    ...
    # Replace the train dataloader's sampler with DistributedSampler so each
    # rank only processes a shard of the data.  DeepSpeed's deepspeed_io()
    # only adds DistributedSampler for eval/predict, not for training.
