...
if [ "$ANALYZE" = "--analyze_gates" ]; then
    deepspeed --bind_cores_to_rank finetune_moe.py --model_name $MODEL --output_dir output --lr 2e-5 --batch_size $BATCH --deepspeed_config $CONFIG --num_train_samples $TRAINSAMPLE --train_dataset_size $TRAIN_DATASET_SIZE --eval_steps 100 --analyze_gates
else
