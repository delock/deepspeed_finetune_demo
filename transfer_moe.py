...
def main(args):
    ...
    if args.dataset:
        ...
        print(f"Loaded custom dataset with {len(dataset)} samples from {args.dataset}")
    else:
        raise ValueError("Dataset file must be specified with --dataset parameter")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, help="Path to the dataset JSON file containing instruction-response pairs")
    parser.add_argument("--prompt", type=str, help="Path to the prompt file in markdown format")
    parser.add_argument("--teacher", type=str, help="Teacher model name (HF model name, local path, or OpenAI API URL)")
